...
async fn create_user(data: web::Data<AppState>, user_dto: web::Json<SignupUserDTO>) -> Result<HttpResponse, AppError> {
    let user = data.di_container.user_service.sign_up(user_dto.into_inner())?;
    Ok(HttpResponse::Created().json(user))
}

#[put("/{id}")]
async fn update_user(data: web::Data<AppState>, path: web::Path<String>, user: web::Json<UpdateUserDTO>) -> Result<HttpResponse, AppError> {
    let id = path.into_inner();
    let user = data.di_container.user_service.update_user(id, user.into_inner())?;
    Ok(HttpResponse::Ok().json(user))
}
